...
#圈x
[rewrite local]
https://pingyouquan.com/tp5/public/index.php/app/gold/adv url script-request-body https://raw.githubusercontent.com/Arisy/Private-Script/master/Scripts/pyq.js


#loon
http-request https://pingyouquan.com/tp5/public/index.php/app/gold/adv script-path=https://raw.githubusercontent.com/Arisy/Private-Script/master/Scripts/pyq.js, requires-body=true, timeout=10, tag=评有圈


#surge
评有圈 = type=http-request,pattern=https://pingyouquan.com/tp5/public/index.php/app/gold/adv,requires-body=1,max-size=0,script-path=https://raw.githubusercontent.com/Arisy/Private-Script/master/Scripts/pyq.js,script-update-interval=0
...
if($request&&$request.url.indexOf("adv")>=0) {
   const pyqUA = $request.headers['User-Agent']
    ...
    $.log(`[${Arisy}] 获取pyqUA请求: 成功,pyqUA: ${pyqUA}`)
    $.msg(`pyqUA${status}: 成功🎉`, ``)
}
if($request&&$request.url.indexOf("adv")>=0) {
   const pyqad = $request.body
    if(pyqad)    $.setdata(pyqad,`pyqad${status}`)
    $.log(`[${Arisy}] 获取pyqad请求: 成功,pyqad: ${pyqad}`)
    $.msg(`pyqad${status}: 成功🎉`, ``)
}
}
